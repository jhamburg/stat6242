...
plot(x, y)
```
There is a positive linear relationship between the two formulas.

```{r e}
# e)
yhat <- lm(y ~ x)
```
B0 and B1 are close but not exactly -1 and .5. In a surprising note, 
the R square is only .83, which seems low based on the simple model.
